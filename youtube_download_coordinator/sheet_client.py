...
class SheetClient:
    ...
    def __init__(self, config: Config):
        ...
        try:
            ...
            self.dead_letter_sources_worksheet: Worksheet = spreadsheet.worksheet(config.dead_letter_sources_worksheet_name)
            self.dead_letter_tasks_worksheet: Worksheet = spreadsheet.worksheet(config.dead_letter_tasks_worksheet_name)
    ...
    def add_source(self, url: str, accent: str, source_type: str, speaker_gender: str = None, multi_speaker_percentage: float = None):
        """
        ...
        try:
            new_row = [
                ...
                source_type,
                speaker_gender or '',
                multi_speaker_percentage if multi_speaker_percentage is not None else '',
                ''   # ErrorMessage
            ]
    ...
    def move_task_to_dead_letter(self, row_id: int):
        """
        Moves a task row from the video tasks worksheet to the dead-letter tasks worksheet.
        """
        ...
        if not row_to_move:
            logger.warning(f"Task row with ID {row_id} not found.")
            return

        try:
            self._wait_for_api()
            self.dead_letter_tasks_worksheet.append_row(row_to_move, value_input_option='USER_ENTERED')

            self._wait_for_api()
            self.video_tasks_worksheet.delete_rows(row_index)
            logger.info(f"Successfully moved task row with ID {row_id} to the dead-letter tasks queue.")
        except APIError as e:
            logger.error(f"API Error moving task row to dead-letter queue: {e}")
            raise


    def move_source_to_dead_letter(self, row_id: int):
        """
        Moves a source row from the sources worksheet to the dead-letter sources worksheet.
        """
        ...
        try:
            self._wait_for_api()
            self.dead_letter_sources_worksheet.append_row(row_to_move, value_input_option='USER_ENTERED')

            self._wait_for_api()
            self.sources_worksheet.delete_rows(row_index)
            logger.info(f"Successfully moved source row with ID {row_id} to the dead-letter sources queue.")
